...
public class mas implements ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
		...
		} else if ("Compute".equals(event.getActionCommand())) {
			doCompute(textinField.getText(),textoutField.getText());
		}
	}

	private void doCompute(String infile,String outfile) {
		fbaReader = new FBAreader(infile,outfile);
		...
		System.out.println("Examining reactant no " + z + " which is " + fbaReader.reactions[z].name);
		for(int k = 0;k < answer.length;k++) {
			if(fbaReader.S[z][k] != 0) {
				if(Math.abs(answer[k]) > 0.01) {
					System.out.println("ACTIVE: " + fbaReader.reactions[k].name + "," + fbaReader.reactions[k].name+ "," + answer[k]);
				}
				else {
					System.out.println("INACTIVE: " + fbaReader.reactions[k].name + "," + fbaReader.reactions[k].name+ "," + answer[k]);
				}
			}	 
		} 
